...
public class RetailerService {
    ...
    private String getBaseRev(Retailer retailer) {
        return "SELECT a.itemId retailerItemId, "+retailer.fixedUpc()+" as itemId, "+retailer.mapJoin()+" as mapJoin, p.description as description, p.manufacturer as manufacturer, p.deptDescription as deptDescription, a.totalVal as totalVal FROM (SELECT itemId, SUM(ABS(quantity)*ABS(extendedAmount/1000)) totalVal FROM ["+retailer.projectId()+":default.LineItem] GROUP BY 1) a JOIN ["+retailer.projectId()+":default.Product] p ON a.itemId=p.upc";
    }
}
